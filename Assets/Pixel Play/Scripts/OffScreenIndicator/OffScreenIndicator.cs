...
public class OffScreenIndicator : MonoBehaviour
{
    ...
    void DrawIndicators()
    {
        ...
        {
            Vector3 screenPosition = OffScreenIndicatorCore.GetScreenPosition(cc, target.transform.position);
            bool isTargetVisible = OffScreenIndicatorCore.IsTargetVisible(screenPosition);
            float distanceFromCamera = target.NeedDistanceText ? target.GetDistanceFromCamera(cc.transform.position) : float.MinValue;// Gets the target distance from the camera.
